// src/components/units/masjid/Footer.tsx
'use client';

import React from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { Facebook, Instagram, Youtube, MessageCircle, Phone, MapPin, Clock, Heart, BookOpen, Copy, Check } from 'lucide-react';
...
export function MasjidFooter() {
  ...
  const kajianLinks = [
    { name: 'Sabtu Subuh - Riyadhus Shalihin', href: '/kajian#sabtu' },
    { name: 'Ahad - Aqidah & Tarjim', href: '/kajian#ahad' },
    { name: 'Selasa Maghrib - Tahsin', href: '/kajian#selasa' },
    { name: 'Rabu Maghrib - Tadabbur', href: '/kajian#rabu' },
  ];

  return (
    <footer className="bg-gradient-to-br from-[#006064] to-[#00363a] text-white print:hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="col-span-1">
            <Link href="/" className="inline-flex items-center space-x-3 mb-6">
              ...
              <div className="flex flex-col">
                <span className="text-xl font-bold text-white">Masjid Al Muhajirin</span>
                <span className="text-sm text-[#80DEEA]">Rewwin, Waru</span>
              </div>
            </Link>
            <p className="text-[#B2EBF2] mb-6 text-sm leading-relaxed">
              Memakmurkan masjid dengan berbagai program keagamaan, kajian rutin, dan pelayanan umat.
              Bersama membangun jamaah yang beriman, berilmu, dan beramal shalih.
            </p>
          ...
          {/* Kajian Schedule */}
          <div>
            <h3 className="text-lg font-semibold text-white mb-4">Jadwal Kajian</h3>
            <ul className="space-y-3">
              {kajianLinks.map((link) => (
                <li key={link.name}>
                  <Link href={link.href} className="text-[#B2EBF2] hover:text-white text-sm transition-colors flex items-center space-x-2">
                    <BookOpen className="h-4 w-4 text-[#FFB300]" />
                    <span>{link.name}</span>
                  </Link>
                </li>
              ))}
            ...
            <div className="mt-6 p-4 bg-white/10 rounded-xl">
              <p className="text-sm font-semibold text-[#FFB300] mb-2">Rekening Infaq</p>
              <p className="text-white text-sm">BSI: 719.5656.578</p>
              <p className="text-[#B2EBF2] text-xs">a/n Infaq Masjid Al-Muhajirin Rewwin</p>
              <button
                onClick={handleCopyRekening}
                className="mt-2 flex items-center space-x-2 text-xs text-[#80DEEA] hover:text-white transition-colors"
              >
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-start space-x-3 text-sm">
                <Clock className="h-4 w-4 text-[#4CAF50] mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-[#B2EBF2]">Jam Operasional:</p>
                  <p className="text-white">
                    Masjid: 24 Jam<br />
                    Sekretariat: Senin - Jumat
                  </p>
                </div>
              </li>
            </ul>
        ...
        <div className="border-t border-white/20 mt-8 pt-8">
          <div className="bg-gradient-to-r from-[#00BCD4]/20 to-[#4CAF50]/20 rounded-2xl p-6 text-center">
            ...
            <p className="text-[#B2EBF2] mb-4">
              Setiap langkah ke masjid adalah pahala. Hadirilah kajian rutin dan jadilah bagian dari jamaah yang aktif.
            </p>
        ...
        <div className="border-t border-white/20 mt-8 pt-8 flex flex-col md:flex-row md:justify-between items-center">
          <p className="text-[#B2EBF2] text-sm">
            &copy; {currentYear} Masjid Al Muhajirin Rewwin. All rights reserved.
          </p>
