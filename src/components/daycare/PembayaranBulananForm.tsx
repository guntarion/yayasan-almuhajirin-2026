...
export interface TagihanBulananForPayment extends TagihanBulananWithRelations {
  _totalBayar: number;
  _sisaTagihan: number;
}

interface PembayaranBulananFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  tagihan: TagihanBulananForPayment | null;
  onSuccess?: () => void;
}

export function PembayaranBulananForm({
  open,
  onOpenChange,
  tagihan,
  onSuccess,
}: PembayaranBulananFormProps) {
  ...
  const totalTagihan = tagihan ? Number(tagihan.totalTagihan) : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/daycare/pembayaran-bulanan', {
        method: 'POST',
  ...
  const isFullyPaidAfterThis = (totalBayar + Number(formData.nominal || 0)) >= totalTagihan;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Tagihan {getNamaBulan(tagihan.bulan)} {tagihan.tahun} - {tagihan.anak?.namaLengkap}
          </DialogDescription>
        </DialogHeader>
        ...
        {sisaTagihan > 0 && isFullyPaidAfterThis && formData.nominal && Number(formData.nominal) > 0 && (
          <div className="flex items-center gap-2 p-3 bg-cyan-50 border border-cyan-200 text-[#006064] rounded-md text-sm">
            <CheckCircle className="w-4 h-4 flex-shrink-0" />
            <span>Pembayaran ini akan melunasi tagihan.</span>
          </div>
        )}
